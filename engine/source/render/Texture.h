...
namespace RayTracer
{
    ...
    class Texture
    {
        public:
            ...
            Texture(int width, int height, std::unique_ptr<Color[]> pBuffer, int channels);
    ...
    {
        public:
            TexChecker(float _scale, std::shared_ptr<SpatialTexture> _even, std::shared_ptr<SpatialTexture> _odd)
    ...
    class NoiseTexture : public SpatialTexture
    {
        public:
            ...
            {
                auto s = scale * p;
                return Vector3(1.0f, 1.0f, 1.0f) * 0.5 * (1.0f + sinf(s.z + 10.0f * perlin.Turb(s)));

                // return Vector3(1.0f, 1.0f, 1.0f) * 0.5f * (1.0f + perlin.Noise(scale * p));
            }
        private:
            float scale;
            Perlin perlin;
