...
Texture* Camera::Render(const Hittable& world, unsigned int samples_per_pixel) {
    ...
    for (unsigned int j = 0; j < height; ++j) {
        ...
        for (unsigned i = 0; i < width; ++i) {
            ...
            // for (unsigned int sample = 0; sample < samples_per_pixel;
            // ++sample) {
            //     Ray r =samples_per_pixel > 1? get_ray(i, j) : Ray(cameraPos,
            //     ray_direction); pixel_color += ray_color(r, world,
            //     max_depth);
            // }
...
void Camera::Initialize() {
    ...
    viewport_upper_left =
        cameraPos + forward() * focus_dist - 0.5f * (viewport_u + viewport_v);
    pixel00_loc = viewport_upper_left + 0.5f * (pixel_delta_u + pixel_delta_v);

    // Calculate the defocus disk
    auto defocus_radius =
        focus_dist * tan(DegreeToRadian(defocus_angle) * 0.5f);
...
Vector3 Camera::ray_color(const Ray& r, const Hittable& world, int depth) {
    ...
    float pdf{};
    if (!result.hit_material->Scatter(r, result, scatter)) return emitted;

    if (scatter.skip_pdf)
        return emitted +
               scatter.albedo * ray_color(scatter.scatter_ray, world, --depth);

    if (Lights::lights->objects.size() > 0) {
        MixturePdf mixed_pdf(
            std::make_shared<HittablePdf>(Lights::lights, result.hit_point),
            scatter.pdf);
        scatteredRay =
            Ray(result.hit_point,
                mixed_pdf.generate());  // get a random ray to the light
        pdf = mixed_pdf.Value(
            scatteredRay.direction());  // get the pdf of the ray
    } else {
        scatteredRay =
            Ray(result.hit_point,
                scatter.pdf->generate());  // get a random ray to the light
        pdf = scatter.pdf->Value(
            scatteredRay.direction());  // get the pdf of the ray
    }
    if (pdf < kEpsilon) return emitted;

    // very hard coded light source
    // auto on_light = Vector3(random_float(213.0f , 343.0f ), 554.0f ,
    // random_float(-332.0f, -227.0f)); auto to_light = on_light -
    // result.hit_point; auto distance_squared = to_light.lengthSqr(); to_light
    // = to_light.normalized();
