...
Texture::Texture(int width, int height, std::unique_ptr<Color[]> pBuffer,
                 int channels) {
...
std::shared_ptr<Texture> Texture::LoadFrom(const std::string name) {
    ...
    int channels;
    unsigned char *image =
        stbi_load(name.c_str(), &width, &height, &channels, STBI_default);
    if (!image) {
    ...
    std::unique_ptr<Color[]> pBuffer =
        std::make_unique<Color[]>(width * height);
    ...
    return std::make_shared<Texture>(width, height, std::move(pBuffer),
                                     channels);
}
}  // namespace RayTracer