...
class Translation : public Hittable {
   public:
    ...
    bool Hit(const Ray& r, Interval t_interval, HitResult& res) const {
        ...
        if (!object->Hit(offset_r, t_interval, res)) return false;
    ...
    static std::shared_ptr<Hittable> Translate(std::shared_ptr<Hittable> p,
...
class Rotation : public Hittable {
   public:
    Rotation(std::shared_ptr<Hittable> p, const Vector3& rotate)
        ...
        rotMatrixInv =
            rotMatrix
                .Transpose();  // inverse of rotation matrix is its transpose
        auto rotMinOffset = (rotMatrix * Vector4(minOffset, 1.0f)).xyz();
        auto rotMaxOffset = (rotMatrix * Vector4(maxOffset, 1.0f)).xyz();

        bounds.SetMinMax(rotMinOffset + bounds.GetCenter(),
                         rotMaxOffset + bounds.GetCenter());
    }
    ...
    bool Hit(const Ray& r, Interval t_interval, HitResult& res) const {
        ...
        Ray rotated_ray =
            Ray((rotMatrixInv * Vector4(r.origin(), 1.0f)).xyz(),
                (rotMatrixInv * Vector4(r.direction(), 0.0f)).xyz());

        // Determine where (if any) an intersection occurs along the offset ray
        if (!object->Hit(rotated_ray, t_interval, res)) return false;
    ...
    static std::shared_ptr<Hittable> Rotate(std::shared_ptr<Hittable> p,
