...
namespace RayTracer
{
    ...
    public:
        ...
        : boundary(b), neg_inv_density(-1/density), phase_function(std::make_shared<Isotropic>(color))
        {
        }
        
        // ConstantMedium(std::shared_ptr<Hittable> b, float density, std::shared_ptr<Material>& mat)
        // : boundary(b), neg_inv_density(-1/density), phase_function(std::make_shared<Isotropic>(mat))
        // {
        // }
        ...
        bool Hit(const Ray& r, Interval t_interval, HitResult& res) const override
        {
            ...
            HitResult res1, res2;
            if (!boundary->Hit(r, Interval::universe, res1))
                return false;
