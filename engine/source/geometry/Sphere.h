...
namespace RayTracer
{
    ...
    {
    public:
        Sphere(const Vector3 center, float radius, std::shared_ptr<Material> mat = nullptr)
        ...
        bool Hit(const Ray &r, Interval t_interval, HitResult &result) const override
        {
         ...
         * @param dir: the direction(normal) of the hit point, centerer at sphere center
         * @return Vector2
         */
        ...
        float pdfValue(const Vector3 &origin, const Vector3 &dir) const
        {
            ...
            float cos_theta_max = sqrtf(1.0f - m_radius * m_radius / (m_center - origin).lengthSqr());
        ...
        {
            Vector3 direction = m_center - hit_point;   // ray is from hit point to sphere center(here sphere is a light)
