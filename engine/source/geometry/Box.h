...
class Box : public Scene {
   public:
    Box(Vector3 center, Vector3 size, std::shared_ptr<Material> mat) {
        ...
        auto front = std::make_shared<Quad>(leftbottom, Vector3::right * size.x,
                                            Vector3::up * size.y, mat);
        auto back = std::make_shared<Quad>(righttop, -Vector3::right * size.x,
                                           -Vector3::up * size.y, mat);
        auto left = std::make_shared<Quad>(
            leftbottom, Vector3::forward * size.z, Vector3::up * size.y, mat);
        auto right = std::make_shared<Quad>(
            righttop, -Vector3::forward * size.z, -Vector3::up * size.y, mat);
        auto top = std::make_shared<Quad>(righttop, -Vector3::right * size.x,
                                          -Vector3::forward * size.z, mat);
        auto bottom =
            std::make_shared<Quad>(leftbottom, Vector3::right * size.x,
                                   Vector3::forward * size.z, mat);
    ...
    //     {
    //         if (side->Hit(r, Interval(t_interval.min, closest_so_far),
    //         result))
    //         {
    //             hit_anything = true;
