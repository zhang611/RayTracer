...
struct Bounds {
   public:
    Bounds() : m_Center(Vector3::zero), m_Extents(Vector3::zero) {}
    ...
    Bounds operator+(const Vector3& offset) const {
        return Bounds(m_Center + offset, m_Extents * 2);
    }
    friend Bounds operator+(const Vector3& offset, const Bounds& bounds) {
