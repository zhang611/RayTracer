...
inline void Partition_NAIVE(std::vector<std::shared_ptr<Hittable>>& objects,
                            std::vector<std::shared_ptr<Hittable>>& left,
                            std::vector<std::shared_ptr<Hittable>>& right) {
...
std::shared_ptr<BVHNode> BVHAccel::Build(const Scene& scene,
                                         SplitMethod splitMethod) {
    if (scene.objects.size() == 0) return nullptr;
    time_t start, stop;
    time(&start);
    ...
    std::cout << "BVH Generation took " << hrs << " hours, " << mins
              << " minutes, " << secs << " seconds\n";
    return root;
}

std::shared_ptr<BVHNode> BVHAccel::recursiveBuild(
    std::vector<std::shared_ptr<Hittable>>& objects, SplitMethod splitMethod) {
...
bool BVHNode::Hit(const Ray& ray, Interval t_interval,
                  HitResult& output_result) const {
    ...
    bool hit_right =
        right != nullptr
            ? right->Hit(ray,
                         Interval(t_interval.min,
                                  hit_left ? output_result.t : t_interval.max),
                         output_result)
            : false;
